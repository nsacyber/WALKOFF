...
class TestUsersAndRoles(ServerTestCase):
    ...
    def test_add_role(self):
        data = {"name": self.name}
        self.put_with_status_check('/roles', data=json.dumps(data), headers=self.headers,
                                   status_code=OBJECT_CREATED, content_type='application/json')

    def test_add_role_already_exists(self):
        data = {"name": self.name}
        self.app.put('/roles', data=json.dumps(data), headers=self.headers, content_type='application/json')
        self.put_with_status_check('/roles', error='Role already exists.', data=json.dumps(data), headers=self.headers,
    ...
    def test_display_all_roles(self):
        ...
        response = self.get_with_status_check('/roles', headers=self.headers, status_code=SUCCESS)
    ...
    def test_edit_role_description(self):
        ...
        response = self.post_with_status_check('/roles', data=json.dumps(data), headers=self.headers,
    ...
    def test_add_user(self):
        ...
        self.put_with_status_check('/users', data=json.dumps(data), headers=self.headers, status_code=OBJECT_CREATED, content_type='application/json')

    def test_add_user_already_exists(self):
        data = {"username": self.email, "password": self.password}
        self.app.put('/users', data=json.dumps(data), headers=self.headers, content_type='application/json')

        self.put_with_status_check('/users', data=json.dumps(data), headers=self.headers,
                                   status_code=OBJECT_EXISTS_ERROR, content_type='application/json')
    ...
    def test_remove_user(self):
        data = {"username": self.email, "password": self.password}
        response = self.app.put('/users', data=json.dumps(data), headers=self.headers,
                     content_type='application/json').get_data(as_text=True)
        user_id = json.loads(response)['id']
        self.delete_with_status_check('/users/{0}'.format(user_id), status_code=SUCCESS, headers=self.headers)

    def test_add_role_to_user(self):
        data = {"username": self.email, "password": self.password}
        response = self.app.put('/users', data=json.dumps(data), headers=self.headers, content_type='application/json')
        ...
        response = self.post_with_status_check(
            '/users', data=json.dumps(data), headers=self.headers, status_code=SUCCESS, content_type='application/json')
    ...
    def test_get_all_users(self):
        ...
        response = self.get_with_status_check('/users', headers=self.headers, status_code=SUCCESS)
