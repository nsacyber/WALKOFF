...
class TestDevicesServer(ServerTestCase):
    ...
    def test_read_device(self):
        device1 = Device('test', [], [], 'type')
        device2 = Device('test2', [], [], 'type')
        self.execution_db.session.add(device2)
        self.execution_db.session.add(device1)
        self.execution_db.session.commit()
    ...
    def test_delete_device(self):
        ...
        self.assertIsNone(
            self.execution_db.session.query(Device).filter(Device.id == device1_id).first())
    ...
    def test_create_device(self):
        ...
        device = self.execution_db.session.query(Device).filter(Device.name == 'test').first()
    ...
    def test_import_apps_devices(self):
        ...
        dev = self.execution_db.session.query(Device).filter(Device.name == "testDevice").first()
        self.execution_db.session.delete(dev)
        self.execution_db.session.commit()
