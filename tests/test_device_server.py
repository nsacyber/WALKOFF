...
import tests.config
from walkoff.executiondb.devicedb import Device, App, DeviceField
...
class TestDevicesServer(ServerTestCase):
    ...
    def tearDown(self):
        ...
        for device in walkoff.executiondb.devicedb.device_db.session.query(Device).all():
            walkoff.executiondb.devicedb.device_db.session.delete(device)
        for field in walkoff.executiondb.devicedb.device_db.session.query(DeviceField).all():
            walkoff.executiondb.devicedb.device_db.session.delete(field)
        app = walkoff.executiondb.devicedb.device_db.session.query(App).filter(App.name == self.test_app_name).first()
        if app is not None:
            walkoff.executiondb.devicedb.device_db.session.delete(app)
    ...
    def test_read_all_devices(self):
        ...
        walkoff.executiondb.devicedb.device_db.session.add(app)
        walkoff.executiondb.devicedb.device_db.session.commit()
        response = self.get_with_status_check('/api/devices', headers=self.headers, status_code=SUCCESS)
    ...
    def test_read_device(self):
        device1 = Device('test', [], [], 'type')
        device2 = Device('test2', [], [], 'type')
        walkoff.executiondb.devicedb.device_db.session.add(device2)
        walkoff.executiondb.devicedb.device_db.session.add(device1)
        walkoff.executiondb.devicedb.device_db.session.commit()
    ...
    def test_delete_device(self):
        ...
        self.assertIsNone(
            walkoff.executiondb.devicedb.device_db.session.query(Device).filter(Device.id == device1_id).first())
    ...
    def test_create_device(self):
        ...
        device = walkoff.executiondb.devicedb.device_db.session.query(Device).filter(Device.name == 'test').first()
    ...
    def test_import_apps_devices(self):
        ...
        dev = walkoff.executiondb.devicedb.device_db.session.query(Device).filter(Device.name == "testDevice").first()
        walkoff.executiondb.devicedb.device_db.session.delete(dev)
        walkoff.executiondb.devicedb.device_db.session.commit()
