...
from core.steptrigger import StepTrigger, _Widget
...
class TestStepTrigger(unittest.TestCase):
    ...
    def test_init_action_only(self):
        step = StepTrigger(action='helloWorld')
        self.__compare_init(step, '', 'helloWorld', [], [])

    def test_init_with_name(self):
        step = StepTrigger(action='helloWorld', name='name')
        self.__compare_init(step, 'name', 'helloWorld', [], [])

    def test_init_with_empty_flags(self):
        step = StepTrigger(action='helloWorld', name='name', flags=[])
        self.__compare_init(step, 'name', 'helloWorld', [], [])
    ...
    def test_from_json_with_name(self):
        ...
        step = StepTrigger.from_json(self.basic_input_json, {})
        self.__compare_init(step, 'name', 'helloWorld', [], [])
    ...
    def test_from_json_with_position(self):
        step = StepTrigger.from_json(self.basic_input_json, {'x': 125.3, 'y': 198.7})
    ...
    def test_call(self):
        ...
        for name, flags, input_str, expect_name in inputs:
            step = StepTrigger(action='helloWorld', flags=flags)
            if expect_name:
