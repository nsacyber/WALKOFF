...
from core.appstep import AppStep, _Widget
...
class TestStepAction(unittest.TestCase):
    ...
    def test_init_with_uid(self):
        uid = uuid.uuid4().hex
        step = AppStep(app='HelloWorld', action='helloWorld', uid=uid)
    ...
    def test_init_invalid_app(self):
        with self.assertRaises(UnknownApp):
            AppStep(app='InvalidApp', action='helloWorld')
    ...
    def test_init_with_inputs_no_conversion(self):
        step = AppStep(app='HelloWorld', action='returnPlusOne', inputs={'number': -5.6})
    ...
    def test_init_with_invalid_input_name(self):
        with self.assertRaises(InvalidInput):
            AppStep(app='HelloWorld', action='returnPlusOne', inputs={'invalid': '-5.6'})

    def test_init_with_invalid_input_type(self):
        with self.assertRaises(InvalidInput):
            AppStep(app='HelloWorld', action='returnPlusOne', inputs={'number': 'invalid'})

    def test_init_with_name(self):
        step = AppStep(app='HelloWorld', action='helloWorld', name='name')
        self.__compare_init(step, 'name', 'helloWorld', 'HelloWorld', '', {}, [], [])

    def test_init_with_device(self):
        step = AppStep(app='HelloWorld', action='helloWorld', device='dev')
        self.__compare_init(step, '', 'helloWorld', 'HelloWorld', 'dev', {}, [], [])

    def test_init_with_risk(self):
        step = AppStep(app='HelloWorld', action='helloWorld', risk=42.3)
        self.__compare_init(step, '', 'helloWorld', 'HelloWorld', '', {}, [], [], risk=42.3)

    def test_init_with_widgets(self):
        widgets = [('aaa', 'bbb'), ('ccc', 'ddd'), ('eee', 'fff')]
        step = AppStep(app='HelloWorld', action='helloWorld', widgets=widgets)
        self.__compare_init(step, '', 'helloWorld', 'HelloWorld', '', {}, [], widgets)

    def test_init_with_position(self):
        step = AppStep(app='HelloWorld', action='helloWorld', position={'x': -12.3, 'y': 485})
    ...
    def test_init_with_next_steps(self):
        ...
        step = AppStep(app='HelloWorld', action='helloWorld', next_steps=next_steps)
        self.__compare_init(step, '', 'helloWorld', 'HelloWorld', '', {}, [step.as_json() for step in next_steps], [])

    def test_as_json(self):
        step = AppStep(app='HelloWorld', action='helloWorld', uid=self.uid)
        self.assertDictEqual(step.as_json(), self.basic_json)

    def test_as_json_with_name(self):
        step = AppStep(app='HelloWorld', action='helloWorld', name='name', uid=self.uid)
        self.basic_json['name'] = 'name'
        self.assertDictEqual(step.as_json(), self.basic_json)

    def test_as_json_with_device(self):
        step = AppStep(app='HelloWorld', action='helloWorld', device='device', uid=self.uid)
        self.basic_json['device'] = 'device'
        self.assertDictEqual(step.as_json(), self.basic_json)

    def test_as_json_with_risk(self):
        step = AppStep(app='HelloWorld', action='helloWorld', risk=120.6, uid=self.uid)
        self.basic_json['risk'] = 120.6
        self.assertDictEqual(step.as_json(), self.basic_json)

    def test_as_json_with_inputs(self):
        step = AppStep(app='HelloWorld', action='returnPlusOne', inputs={'number': '-5.6'}, uid=self.uid)
    ...
    def test_as_json_with_next_steps(self):
        next_steps = [NextStep(), NextStep(name='name'), NextStep(name='name2')]
        step = AppStep(app='HelloWorld', action='helloWorld', next_steps=next_steps, uid=self.uid)
        self.basic_json['next'] = [next_step.as_json() for next_step in next_steps]
        self.assertDictEqual(step.as_json(), self.basic_json)

    def test_as_json_with_position(self):
        step = AppStep(app='HelloWorld', action='helloWorld', position={'x': -12.3, 'y': 485}, uid=self.uid)
        self.basic_json['position'] = {'x': -12.3, 'y': 485}
        self.assertDictEqual(step.as_json(), self.basic_json)

    def test_as_json_with_widgets(self):
        widgets = [('aaa', 'bbb'), ('ccc', 'ddd'), ('eee', 'fff')]
        step = AppStep(app='HelloWorld', action='helloWorld', widgets=widgets, uid=self.uid)
    ...
    def test_from_json_app_and_action_only(self):
        step = AppStep.from_json(self.basic_input_json, {})
        self.__compare_init(step, '', 'helloWorld', 'HelloWorld', '', {}, [], [])
    ...
    def test_from_json_invalid_app(self):
        ...
        with self.assertRaises(UnknownApp):
            AppStep.from_json(self.basic_input_json, {})
    ...
    def test_from_json_with_position(self):
        step = AppStep.from_json(self.basic_input_json, {'x': 125.3, 'y': 198.7})
