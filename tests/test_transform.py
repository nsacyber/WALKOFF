...
class TestTransform(unittest.TestCase):
    ...
    def test_execute_with_args_no_conversion(self):
        self.assertAlmostEqual(Transform(action='mod1_filter2', args={'arg1': '10.3'}).execute('5.4', {}), 15.7)

    def test_execute_with_args_with_conversion(self):
        self.assertAlmostEqual(Transform(action='mod1_filter2', args={'arg1': '10.3'}).execute(5.4, {}), 15.7)

    def test_execute_with_args_with_routing(self):
        self.assertAlmostEqual(Transform(action='mod1_filter2', args={'arg1': '@step1'}).execute(5.4, {'step1': 10.3}),
                               15.7)

    def test_execute_with_complex_args(self):
        original_filter = Transform(action='sub1_filter1', args={'arg1': {'a': '5.4', 'b': 'string_in'}})
    ...
    def test_call_with_nested_complex_args(self):
        ...
        original_filter = Transform(action='complex', args=args)
        self.assertAlmostEqual(original_filter.execute(3, {}), 19.0)

    def test_call_with_args_invalid_input(self):
        self.assertEqual(Transform(action='mod1_filter2', args={'arg1': '10.3'}).execute('invalid', {}), 'invalid')
