...
from api.server.db.role import AddRoleModel, RoleModel


async def role_getter(role_col: AsyncIOMotorCollection, role_id: int):
    return await role_col.find_one({"id_": role_id}, projection={'_id': False})
...
@router.post("/")
async def create_role(add_role: AddRoleModel, role_col: AsyncIOMotorCollection = Depends(get_mongo_c)):
...
@router.get('/{role_id}')
async def read_role(role_id: int, role_col: AsyncIOMotorCollection = Depends(get_mongo_c)):
...
@router.put('/{role_id}')
async def update_role(role_id: int, updated_role: RoleModel, role_col: AsyncIOMotorCollection = Depends(get_mongo_c)):
    role = await role_getter(role_col, role_id=role_id)
    if role.id_ != 1 and role.id_ != 2:
...
async def delete_role(role_id: int, role_col: AsyncIOMotorCollection = Depends(get_mongo_c)):
    role = await role_getter(role_col, role_id=role_id)
    if role.id_ != 1 or role.id_ != 2:
