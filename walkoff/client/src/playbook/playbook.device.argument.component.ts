...
@Component({
	selector: 'playbook-device-argument-component',
	templateUrl: './playbook.device.argument.html',
	styleUrls: [],
	encapsulation: ViewEncapsulation.None,
	providers: [],
})
export class PlaybookDeviceArgumentComponent implements OnInit {
	@Input() id: number;
	...
	ngOnInit(): void {
        ...
        this.parameterSchema.type = 'device';
	...
	initDeviceSelect(): void {
		this.selectData = [{id: '', text: ''}].concat(this.devices.map((device) => {
			return { id: device.id.toString(), text: device.name };
		}));
		...
		if (this.argument.value) this.selectInitialValue = JSON.parse(JSON.stringify(this.argument.value));
	}
	...
	selectChange($event: any): void {
        console.log('changed value to ' + $event.value);
