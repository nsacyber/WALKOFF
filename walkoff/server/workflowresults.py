...
def __workflow_ended_callback(sender, **kwargs):
    ...
    workflow_metric = current_app.running_context.execution_db.session.query(WorkflowMetric).filter_by(workflow_id=sender['id']).first()
...
def __update_action_tracker(status, action_status):
    app_metric = current_app.running_context.execution_db.session.query(AppMetric).filter_by(app=action_status.app_name).first()
