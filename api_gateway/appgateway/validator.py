...
from api_gateway.appgateway.apiutil import InvalidParameter, InvalidApi
from api_gateway.helpers import get_function_arg_names, format_exception_message
...
def validate_actions(actions, dereferencer, app_name):
    from api_gateway.appgateway import get_app_action
    from api_gateway.appgateway import get_all_actions_for_app
...
def validate_app_action_parameters(api, parameters, app, action, accumulator=None):
    message_prefix = 'app {0} action {1}'.format(app, action)
    return validate_parameters(api, parameters, message_prefix, accumulator)


def validate_condition_parameters(api, parameters, condition, accumulator=None):
    return validate_parameters(api, parameters, 'condition {0}'.format(condition), accumulator)


def validate_transform_parameters(api, parameters, transform, accumulator=None):
    return validate_parameters(api, parameters, 'transform {0}'.format(transform), accumulator)
