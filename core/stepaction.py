...
class StepAction(ExecutionElement):
    def __init__(self,
                 name='',
                 action='',
                 app='',
        ...
        if flags is not None and status is not '':
            raise InvalidElementConstructed('Flags and status must be specified in either a next step or '
                                            'trigger step constructor and not an action step constructor')
        self.action = action
        self.app = app
    ...
    def from_json(json_in, position):
        ...
        return StepAction(name=json_in['name'],
    ...
    def __dir__(self):
        return ['name', 'action', 'app', 'device', 'inputs', 'next_steps', 'position', 'widgets', 'risk', 'uid',
                'templated', 'raw_json']